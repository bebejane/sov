	...
	persist<StoreState>(
		(set, get) => ({
			...
			updateData: (data, section) => {
				let d = { ...get().data };
				if (section) d[section] = Array.isArray(data) ? data : { ...d[section], ...data };
				else d = Array.isArray(data) ? data : { ...d, ...data };
			...
			resetKeys: (keys, section) => {
				...
				if (keys)
					keys.forEach((key) =>
						section && d[section]?.[key] !== undefined ? delete d[section][key] : delete d[key],
					);
				else d = defaultState;
